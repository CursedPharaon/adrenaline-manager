...
for event in longpoll.listen():
    if event.type == VkBotEventType.MESSAGE_NEW:
        ...
        if event.from_chat:
            ...
            if is_muted(from_id):
                delete_message(peer_id, message_id)  # Удаляем сообщение
                continue
            ...
            if silence_mode:
                if get_access_level(peer_id, from_id) < 100:
                    delete_message(peer_id, message_id)  # Удаляем сообщение
                    # Отправляем предупреждение в ЛС или не отправляем, чтобы не спамить
                    continue
...
📌 **Совет:** Ответьте на сообщение пользователя или используйте @"""
            ...
            # ТИШИНА - УДАЛЯЕТ СООБЩЕНИЯ НЕ-АДМИНОВ
